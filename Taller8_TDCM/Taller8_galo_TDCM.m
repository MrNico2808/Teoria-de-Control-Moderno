...
Kc_sys = 1/series(series(compensador, G1), 1010);
...
[y1, t1] = step(G1, 0:0.1:120);
[y2, t2] = step(feedback(G_compensado, 1010), 0:0.1:120);
plot(t1, y1, 'b-');
plot(t2, y2, 'r-')
...
plot(P_x, 0, 'rx');
plot(Z_x, 0,'ro');
rlocus(feedback(G_compensado, 1010));

figure(); hold on;
step(feedback(G_compensado, 1010));
figure(); hold on;
plot(real(polos_compensador), imag(polos_compensador), 'rx');
rlocus(G1);
figure(); hold on;
